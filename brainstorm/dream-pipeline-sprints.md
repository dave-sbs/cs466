...
## Sprint 11 — Prototype ergonomics: 9825 driver + real RIFE

**Sprint goal:** Make the prototype loop fast to rerun (single script) and upgrade transitions from crossfade to real motion interpolation via Practical-RIFE. Demo: render poem 9825 twice (crossfade vs RIFE) and visibly observe smoother motion in transitions under RIFE.


| ID      | Title                             | Scope (one commit)                                                                       | Test / validation                                              |
| ------- | --------------------------------- | ---------------------------------------------------------------------------------------- | -------------------------------------------------------------- |
| S11-T1  | `run_dream_9825.py` driver script | `code/scripts/run_dream_9825.py` with `--use-rife` toggle; stream per-stanza logs        | Unit: `--help` parser test; Manual: `%run ...` in Colab        |
| S11-T2a | Implement `RifeInterpolator`      | Replace stub in `code/dream_interp/rife.py` with lazy model load + recursive mids        | Unit: existing `dream_interp` tests updated                    |
| S11-T2b | RIFE tests + setup docs           | Add mocked-recursion unit test + GPU-gated integration test; update `docs/rife_setup.md` | `pytest -m "not gpu"` passes; GPU test skips unless configured |


**Sprint demo (5 min):**

- Crossfade baseline: `%run code/scripts/run_dream_9825.py --force`
- RIFE run: set `RIFE_ROOT` (and weights), then `%run code/scripts/run_dream_9825.py --use-rife --force`
- Compare `dream_poem_9825.mp4` outputs — transitions should look like motion rather than dissolves.
